layui.use(['table','layer'],function(){
    ...
    var tableIns = table.render({
        ...
        ,cols: [[
            ...
            ,{field: 'courseId', title: '课程编号',  sort: true, fixed: 'left'}
            ,{field: 'courseName', title: '课程名称', align:'center'}
            ,{field: 'courseType', title: '所属院系', align:'center'}
            ,{field: 'teacherName', title: '教师姓名', align:'center'}
            ,{field: 'teacherPhone', title: '教师电话', align:'center'}
    ...
    $(".search_btn").click(function () {
        ...
        tableIns.reload({
            ...
            where: { //设定异步数据接口的额外参数，任意设
                // 通过文本框，设置传递的参数
                courseId: $("[name='courseId']").val() // ID
                ,courseType: $("[name='courseType']").val() // 所属院系
                ,teacherName:$("[name='teacherName']").val() // 教师名
            }
            ,page: {
                curr: 1 // 重新从第 1 页开始
            }
        ...
        layer.confirm('您确定要删除选中的记录吗？',{icon:3, title:'用户管理'}, function (index) {
            ...
            for(var i = 0; i < userData.length; i++) {
                if(i < userData.length -1) {
                    ids = ids + userData[i].courseId + "&ids="
                } else {
                    ids = ids + userData[i].courseId;   /* userData中的为实体类中定义的id名courseId */
                }
            }
            // console.log(ids);
    ...
    table.on('tool(users)', function (data) {
        ...
        if (data.event == "edit") { // 更新用户
            ...
            // 打开添加/修改用户的对话框
            openAddOrUpdateUserDialog(data.data.courseId);//跟实体类中的id字段对应

        } else if (data.event == "del") { // 删除用户
            // 删除单条用户记录
            deleteUser(data.data.courseId);
        }
    ...
    function openAddOrUpdateUserDialog(id) {
        var title = "<h3>课程管理 - 添加课程</h3>";
        var url = ctx + "/user/toAddOrUpdateUserPage";

        // 判断id是否为空；如果为空，则为添加操作，否则是修改操作
        if (id != null && id != '') {
            title = "<h3>课程管理 - 更新课程</h3>";
            url+= "?courseId="+id; // 传递主键，查询数据
        }
